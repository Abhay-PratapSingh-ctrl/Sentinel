 ...
 *
 * Runs as a Telegram LongPolling bot.
 * Users link their wallet with: /link 0xWALLET_ADDRESS
 *
 * Alert types:
 *   ⚠️  WARNING  — HF below 130%
 *   🚨  DANGER   — HF below 125%, position paused
 *   🔴  CRITICAL — HF below 120%, emergency rebalance triggered
 *   ✅  RESOLVED — rebalance succeeded
 */
@Slf4j
@Service
public class TelegramAlertService extends TelegramLongPollingBot {
    ...
    public void onUpdateReceived(Update update) {
        ...
        switch (parts[0].toLowerCase()) {
            case "/start", "/help" -> sendMessage(chatId, buildHelpMessage());
            case "/link"           -> handleLinkCommand(chatId, parts);
            case "/unlink"         -> handleUnlinkCommand(chatId);
    ...
    public void sendPausedAlert(VaultPosition position, String txHash) {
        ...
        sendMessage(chatId,
                "🚨 *SENTINEL ALERT: POSITION PAUSED*\n\n"
                ...
                + "✅ *To resume:*\n"
                + "Deposit more DOT to bring your Health Factor above 140%.\n\n"
    ...
    public void sendRebalanceStartedAlert(VaultPosition position, String dotToSell) {
        ...
        sendMessage(chatId,
                "🔴 *SENTINEL: EMERGENCY REBALANCE TRIGGERED*\n\n"
    ...
    public void sendRebalanceFailedAlert(VaultPosition position, String error) {
        ...
        sendMessage(chatId,
                "❌ *SENTINEL: AUTO-REBALANCE FAILED*\n\n"
