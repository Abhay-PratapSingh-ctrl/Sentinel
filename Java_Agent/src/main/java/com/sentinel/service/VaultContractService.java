...
public class VaultContractService {
    ...
    public VaultPosition getPosition(String userAddress) throws Exception {
        // ── Step 1: Prepare potential interfaces ─────────────────
        Function function8 = new Function("getPosition",
        ...
        Function function6 = new Function("getPosition",
                Collections.singletonList(new Address(userAddress)),
                Arrays.asList(new TypeReference<Uint256>() {}, new TypeReference<Uint256>() {},
                              new TypeReference<Uint256>() {}, new TypeReference<Uint256>() {},
                              new TypeReference<Uint256>() {}, new TypeReference<Bool>() {}));

        Function function5 = new Function("getPosition",
                Collections.singletonList(new Address(userAddress)),
                Arrays.asList(new TypeReference<Uint256>() {}, new TypeReference<Uint256>() {},
                              new TypeReference<Uint256>() {}, new TypeReference<Uint256>() {},
                              new TypeReference<Bool>() {}));

        // ── Step 2: Call with largest expected return ─────────────
        String encodedFunction = FunctionEncoder.encode(function8);
        EthCall response = web3j.ethCall(
                Transaction.createEthCallTransaction(guardianCredentials.getAddress(), config.getVaultAddress(), encodedFunction),
                DefaultBlockParameterName.LATEST
        ).send();

        if (response.hasError()) {
            throw new RuntimeException("getPosition failed: " + response.getError().getMessage());
        }

        String rawValue = response.getValue();
        if (rawValue == null || "0x".equals(rawValue)) return null;

        // ABI 8-tuple = 514 chars, 6-tuple = 386 chars, 5-tuple = 322 chars
        if (rawValue.length() >= 514) {
            List<Type> res = FunctionReturnDecoder.decode(rawValue, function8.getOutputParameters());
            return VaultPosition.builder()
                    .userAddress(userAddress)
                    .collateralDOT(((Uint256) res.get(0)).getValue())
                    .collateralUSDT(((Uint256) res.get(1)).getValue())
                    .mintedSUSD(((Uint256) res.get(2)).getValue())
                    .collateralUSD(((Uint256) res.get(3)).getValue())
                    .healthFactor(((Uint256) res.get(4)).getValue())
                    .paused(((Bool) res.get(5)).getValue())
                    .aegisActive(((Bool) res.get(6)).getValue())
                    .activeCollateralRatio(((Uint256) res.get(7)).getValue())
                    .fetchedAt(System.currentTimeMillis())
                    .riskLevel(computeRiskLevel(((Uint256) res.get(4)).getValue(), ((Uint256) res.get(2)).getValue()))
                    .build();
        } else if (rawValue.length() >= 386) {
            List<Type> res = FunctionReturnDecoder.decode(rawValue, function6.getOutputParameters());
            return VaultPosition.builder()
                    .userAddress(userAddress)
                    .collateralDOT(((Uint256) res.get(0)).getValue())
                    .collateralUSDT(((Uint256) res.get(1)).getValue())
            ...
            List<Type> res = FunctionReturnDecoder.decode(rawValue, function5.getOutputParameters());
            return VaultPosition.builder()
                    .userAddress(userAddress)
                    .collateralDOT(((Uint256) res.get(0)).getValue())
                    .collateralUSDT(BigInteger.ZERO)
                    .mintedSUSD(((Uint256) res.get(1)).getValue())
                    .collateralUSD(((Uint256) res.get(2)).getValue())
                    .healthFactor(((Uint256) res.get(3)).getValue())
                    .paused(((Bool) res.get(4)).getValue())
                    .fetchedAt(System.currentTimeMillis())
                    .riskLevel(computeRiskLevel(((Uint256) res.get(3)).getValue(), ((Uint256) res.get(1)).getValue()))
                    .build();
        }
    }
